...
import { Card, Image, Button, Col  ,Form, FormGroup, Label, input} from "react-bootstrap";
import { useDispatch ,useSelector} from "react-redux";
import {
  getPost,
  postuler
} from "../JS/actions/actionPost";
import {
  deleteJob,
  getJob,
  toggleTrue,
...
<React.Fragment>
    <div className="row no-gutters shadow-lg p-2 mb-3 bg-white rounded"  >  
        <div className="col-md-4">
          <img  src={job.logo} className="card-img App-img" alt={job.nameEntreprise} width="120px" height="160px" />
          <h5 className="card-title btn-link text-dark" >{job.nameEntreprise}</h5>
        </div>
          ...
          <div className="card-body">
            ...
            user.role === "candidat" ?(<button type="button" class="btn btn-danger" onClick={() => dispatch(postuler(Post))}>Postuler</button> ):
